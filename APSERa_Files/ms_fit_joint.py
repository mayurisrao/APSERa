...
def chisq_poly (p, *args):
    x, y, e, signal21, domain, additive, joint, do_opt, met = args
    wtt = 1/e**2
    ...
    if nterms <= 3:
        return ( ((model-y)**2  )*wtt ).sum() / wtt.sum() 
    else :
        MM = 2
    ...
    if met=='fmin':
        ...
        return wtt*np.abs(y-model)
    else:
        raise ValueError("Specified method not implemeneted!")
##########################################################################

def ms_fit(frq0, dat0, **kwargs):
    ...
    pd['error'], pd['xtol'], pd['ftol'], pd['domain'] = np.ones(len(frq0)), 1e-6, 1e-6, 'lin_lin'
    ...
    pd['joint'], pd['signal'], pd['add_ini'], pd['op_file'], pd['op_name'] = False, [False], 0.0, True, "output_file.pickle"

    all_keys = ['error', 'xtol', 'ftol', 'domain', 'maxiter', 'maxfev', 'algo', 'norder', 'temp',\
            'stepsize','niter','basin_iter_loops','xmin','xmax','rescale','seed','additive','joint','signal','add_ini', 'op_file', 'op_name']
    ...
    p_leastsq_full = leastsq(chisq_poly, p1_basin, args=args, ftol=pd['ftol'], xtol=pd['xtol'], maxfev = int(pd['maxfev']), full_output=True)
