...
allfile=dir('D:\Keerthi\DIPOLE_2GHZ_dualpol_TEST_broadband_WITH slot_9_with sma_finitegroundplane3_surfacecurrents - slots19\Export\3d\*[1].txt*');
for i=1:size(allfile,1)
    basefile1=allfile(i).name;
    filena1=fullfile('D:\Keerthi\DIPOLE_2GHZ_dualpol_TEST_broadband_WITH slot_9_with sma_finitegroundplane3_surfacecurrents - slots19\Export\3d',basefile1);
...
for i=1:size(allfile,1)
    ...
    efieldxtopshaped(:,:,i)=(reshape(efieldxcomplextop(:,i),size(theta,2),size(phi,2)));
    efieldytopshaped(:,:,i)=(reshape(efieldycomplextop(:,i),size(theta,2),size(phi,2)));
    efieldztopshaped(:,:,i)=(reshape(efieldzcomplextop(:,i),size(theta,2),size(phi,2)));
...
k=1;
zs=-0.0245;  %% this defines the z axis location of the equivalent current (this should be zero as we have assumed the equivalent surface current plane being at origin)
...
%  fileID = fopen('samplebottom_dipole.txt', 'w');  % 'w' for writing (creates or overwrites the file)
% 
...
n=size(nodelist,1);
   % threshold=3;
for k=1:1:size(freq,2)   % this loop defines the frequency points. This can be changed according to the range of calculation (i.e either one frequency or multiple frequency point calcs)
    ...
    for i=1:size(xloc,1)
        for j=1:size(nodelist,1)
            ...
            Q22t(i,j)=((cosd(ptop(i))*constant3*((dy2*constant1)-constant2))-(sind(ptop(i))*constant3*(dxdy)*constant1))*cellarea/4/pi;
            
        end
    end
   % Q21t(abs(Q21t)>threshold)=0;
   % Q22t(abs(Q22t)>threshold)=0;
   % Q11t(abs(Q11t)>threshold)=0;
   % Q12t(abs(Q12t)>threshold)=0;
...
nodelisthi3=nodelist;
    save('D:\Keerthi\DIPOLE_2GHZ_dualpol_TEST_broadband_WITH slot_9_with sma_finitegroundplane3_surfacecurrents - slots19 \Export\3d\current_bottom.mat','Jhi3','nodelisthi3'); 
